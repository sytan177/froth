...
class ModelRegistry:    
    ...
    def list_models(self, tags: Optional[List[str]] = None, 
                    ...
                    display: bool = True) -> Dict[str, Dict]:
        ...
        if display:
            print("{:<15} {:<3} {:<35} {:<12} {:<20}".format("Name", "Type", "Features", "Trained", "Config"))
            print("-"*90)
            for name, info in models.items():
                desc = info['description']
                type_ = "3D" if desc.lower().startswith("3d") else "2D"
                features = desc.split(",", 1)[-1].strip() if "," in desc else ""
                features_display = (features[:32] + '…') if len(features) > 32 else features
                print("{:<15} {:<3} {:<35} {:<12} {:<20}".format(
                    name, type_.upper(), features_display, info['training_date'], info['config']
                ))
    ...
    def get_model_path(self, model_name: str) -> Path:
        ...
        return data_dir / 'models' / checkpoint_rel