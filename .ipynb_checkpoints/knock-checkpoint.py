import h5py, time, os, sys, joblib, sys
import json
import hashlib
import numpy as np
...
from .utils import dict_indexing, build_adj
...
class BubbleExtractor:
    def __init__(self, data, 
        ...
        ----------
        snap_data : SnapData
            SnapData object containing gas particles
        ...
        inner_params : dict, optional
            ...
            - component_iteration_size_factor : int, default=0
              Minimum component size increases with iterations: size_limit = 5 + factor * iteration.
        ...
        shell_params : dict, optional
            ...
            - pair_order (str): 'density' or 'density_temperature' (default: 'density')
            - chunk_size (int): Batch size for parallelization (default: 1000000)
    ...
    def _config_hash(self):
        config_dict = {'inner': self.inner_params}
        if not self.skip_shell:
            config_dict['shell'] = self.shell_params
...
class BubbleShellAssignment:
    def __init__(self, data, save_results = True, **kwargs):
        self._data = data
        if isinstance(self._data, SnapData):
            self._get_vor = lambda: data.vor
